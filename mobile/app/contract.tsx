...
// ---------------------------------------------------------------------------
// Inline contract data (will be replaced with import from data/contractData.ts)
// ---------------------------------------------------------------------------
...
function SectionCard({
  ...
  return (
    <View className="mx-4 mb-3 bg-white rounded-2xl border border-slate-200 overflow-hidden">
      {/* Header */}
      <Pressable
        onPress={onToggle}
      ...
      {expanded && (
        <View className="px-4 pb-4 pt-1 border-t border-slate-100">
          {section.items.map((item, idx) => (
            <View key={idx} className={idx > 0 ? 'mt-4' : 'mt-2'}>
              {item.heading && (
              ...
              {item.body && (
                <Text className="text-sm text-slate-500 leading-5 mb-1">
                  {item.body}
                </Text>
              )}

              {item.table && (
                <View className="bg-slate-50 rounded-xl mt-1 overflow-hidden">
              ...
              {item.list && (
                <View className="mt-1">
                  {item.list.map((entry, listIdx) => (
                    <View key={listIdx} className="flex-row mt-1.5">
                      <Text className="text-sm text-blue-500 mr-2 mt-px">
                        {'\u2022'}
                      </Text>
                      <Text className="text-sm text-slate-600 flex-1 leading-5">
                        {entry}
                      </Text>
                    </View>
                  ))}
...
export default function ContractScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
      {/* Header */}
      <View className="px-4 py-3 flex-row items-center justify-between border-b border-slate-200 bg-white">
        <Pressable
          onPress={() => router.back()}
      ...
      >
        {/* Intro banner */}
        <View className="mx-4 mt-4 mb-2 bg-blue-600 rounded-2xl p-5 overflow-hidden">
          <View
        ...
        {/* Expand / Collapse all */}
        <View className="flex-row justify-end mx-4 mb-2 mt-1">
          <Pressable
            onPress={toggleAll}
            className="flex-row items-center gap-1 px-3 py-1.5 rounded-full bg-slate-100 active:bg-slate-200"
          >
            <Ionicons
        ...
        {/* Disclaimer */}
        <View className="mx-4 mt-2 mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
